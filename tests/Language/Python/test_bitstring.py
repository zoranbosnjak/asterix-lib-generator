...
def test_join2(lst : List[bytes]) -> None:
    ...
    b = Bits.join([Bits.from_bytes(bs) for bs in lst])
    assert a == b
...
def test_single_bit_uinteger(ix : int) -> None:
    bs = Bits(bs=b'\xff\x00', bit_offset=ix, bit_size=1)
...
def test_parse_unparse(lst1 : List[int], b : int, lst2 : List[int]) -> None:
    ...
    assert Bits.join([b1,b2,b3]) == bs.take(a+b+c)

    b2new = Bits.from_uinteger(b2.to_uinteger(), a%8, b)
    assert b2new == b2
    assert Bits.join([b1,b2new,b3]) == bs.take(a+b+c)
